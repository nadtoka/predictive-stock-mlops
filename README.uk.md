...
2. Continuous Training Engine (train.py та train_xgb.py):
   ...
   * Глибокий Feature Engineering (Матриця з 20 ознак): Система динамічно збирає технічні, календарні, макроекономічні та фундаментальні фактори.
   * Multi-Output Навчання: Обидві моделі працюють у режимі багатоцільової регресії. Модель вчиться на відсоткових доходностях (pct_change) і за один прохід прогнозує вектор із трьох значень: рух на 1 день наперед (завтра), кумулятивний рух на 5 днів наперед (робочий тиждень) та на 20 днів наперед (місячний бізнес-тренд).
...
3. Continuous Evaluation Engine (evaluate.py та evaluate_xgb.py):
   ...
   * Рахує математичну похибку моделей (MAE у відсотках та доларах) та Directional Accuracy (Win Rate) для трьох часових горизонтів (1d, 5d та 20d).
...
4. Public Client Inference (predict.py):
   ...
   * Працює без токенів: стягує останні зафіксовані ваги моделей з Hugging Face Hub, локально прораховує аналогічний математичний граф фіч для поточної дати за останні 2 роки історії (period="2y") і миттєво виводить потрійний прогноз у консоль: **1д, 5д та 20д**.

---

## 📊 Матриця вхідних ознак (20 Feature Columns)
...
| Моментум та волатильність | `RSI_14` | Індекс відносної сили для детекції зон перегріву |
| | `Distance_to_MA200` | Відстань ціни від глобального річного тренду (200-денна середня) |
| | `ATR_Ratio` | Нормалізований індикатор цінового шоку (True Range / ATR_14) |
...
| | `Analyst_Upside` | Динамічний консенсус-апсайд Волл-стріт ((targetMeanPrice - Close) / Close) |
| | `PE_Expansion` | Коефіцієнт форвардного росту прибутку (forwardPE / trailingPE) |
| | `Analyst_Score` | Консенсусний скоринг аналітиків (recommendationMean від 1.0 до 5.0) |
