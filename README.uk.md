...
У версії **v3.1** система отримала контур Continuous Learning з адаптивним коригуванням зсуву помилок та бейджами довіри, розширивши автономний цикл оцінювання більшою зворотнім зв'язком у продакшні та надійністю операцій.
...
2. Continuous Training Engine (train.py):
   ...
   * **Dynamic Bias Correction:** Автоматично розраховує медіанний зсув похибки (Bias) за останні 14 днів і компенсує прогнози з капінгом ±3% від поточної ціни.
...
3. Continuous Evaluation Engine (evaluate.py):
   ...
   * Завантажує лог прогнозів predictions_history.csv, групує запити до Yahoo Finance для збору фактичних цільових цін закриття (Ground Truth).
   ...
   * Автоматично версіонує розширену матрицю метрик в evaluation_history.csv на Hugging Face Datasets та надсилає деталізований репорт у Telegram, розбитий по рядках для обходу лімітів Telegram API.
...
├── evaluate.py             # Модуль автоматичного аудиту точності, розрахунку MAE % та Win Rate
